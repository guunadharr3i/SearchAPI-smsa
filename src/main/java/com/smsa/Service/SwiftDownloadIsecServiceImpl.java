 ...
 * @author abcom
 */
@Service("isec_download")
public class SwiftDownloadIsecServiceImpl implements SmsaDownloadService{

    private static final Logger logger = LogManager.getLogger(SwiftDownloadIsecServiceImpl.class);

    @Autowired
    SwiftIsecServiceImpl mainService;
    ...
    public List<SmsaDownloadResponsePojo> filterDownloadData(SwiftMessageHeaderFilterPojo filter) {
        ...
        try {
            ...
            logger.info("✅ Retrieved {} records from SwiftMessageHeaderIsec", resultList.size());
    ...
    public Map<Long, String> getAllMessages() {
        ...
        ScrollableResults scroll = entityManager.unwrap(Session.class)
                .createNativeQuery(
                        ...
                        + "LEFT JOIN ISEC_PRT_MESSAGE_HDR h ON i.SMSA_MESSAGE_ID = h.SMSA_MESSAGE_ID "
                        + "LEFT JOIN ISEC_MSG_TXT t ON i.SMSA_MESSAGE_ID = t.SMSA_MESSAGE_ID "
                        + "LEFT JOIN ISEC_MSG_TRL tr ON i.SMSA_MESSAGE_ID = tr.SMSA_MESSAGE_ID"
                )
